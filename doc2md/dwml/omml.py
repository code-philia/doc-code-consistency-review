...
class Pr(Tag2Method):
	...
	def do_common(self,elm):
		...
		if stag in self.__val_tags:
			t = None
			k = elm.get('{0}val'.format(OMML_NS))
			s = elm.get('val')
			if k:
				t = k
			elif s:
			...
			# t = elm.attrib['val']
			if t != None:
				t = t if isinstance(t, str) else t.encode("unicode_escape").decode("ascii")
				store = CHR_BO
				if store.get(t):
					return t
...
class oMath2Latex(Tag2Method):
	...
	def do_nary(self,elm):
		...
		for stag,t,e in self.process_children_list(elm):
			if stag == 'naryPr':
				...
				bo = get_val(t, store=CHR_BO)
			else :
				res.append(t)
		if res:
			return str(bo)+BLANK.join(res)
		else:
			return str(bo)
	...
	def do_r(self,elm):
		...
		"""
		_str = []
		info = elm.findtext('./{0}t'.format(OMML_NS))
		if info:
